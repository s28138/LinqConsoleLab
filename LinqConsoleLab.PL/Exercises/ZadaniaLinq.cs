...
public sealed class ZadaniaLinq
{
    ...
    public IEnumerable<string> Zadanie04_PierwszyPrzedmiotAnalityczny()
    {
        ...
        [
            przedmiot is null
                ? "Nie udało znaleźc się przedmiotu z kategorii Analytics."
                : $"{przedmiot.Nazwa} | start: {przedmiot.DataStartu:yyyy-MM-dd}"
        ];
    }
    ...
    {
        var czyIstnieje = DaneUczelni.Zapisy.Any(z => !z.CzyAktywny);
            return [$"Czy istnieje nieaktywny zapis: {(czyIstnieje ? "Tak" : "Nie")}"];
    }
    ...
    {
            var czyWszyscyMajaKatedre = DaneUczelni.Prowadzacy
                .All(p => !string.IsNullOrWhiteSpace(p.Katedra));
            return [$"Czy wszyscy prowadzący mają katedrę: {(czyWszyscyMajaKatedre ? "Tak" : "Nie")}"];
    }
    ...
    public IEnumerable<string> Zadanie07_LiczbaAktywnychZapisow()
    {
            ...
            return [$"Liczba aktywnych zapisów: {liczbaAktywnychZapisow}"];
    }
    ...
    public IEnumerable<string> Zadanie16_NajwyzszaOcenaKazdegoStudenta()
    {
        return DaneUczelni.Studenci
            .Join(
                ...
                (student, zapis) => new
                {
                    ImieNazwisko = $"{student.Imie} {student.Nazwisko}",
                    Ocena = zapis.OcenaKoncowa!.Value
                })
            .GroupBy(x => x.ImieNazwisko)
            .OrderBy(grupa => grupa.Key)
            .Select(grupa => $"{grupa.Key} | najwyższa ocena: {grupa.Max(x => x.Ocena):0.0}");
    }
    ...
    public IEnumerable<string> Wyzwanie01_StudenciZWiecejNizJednymAktywnymPrzedmiotem()
    {
        return DaneUczelni.Studenci
                ...
                .OrderBy(grupa => grupa.Key)
                .Select(grupa => $"{grupa.Key} | aktywne przedmioty: {grupa.Count()}");
    }
    ...
    public IEnumerable<string> Wyzwanie03_ProwadzacyISredniaOcenNaIchPrzedmiotach()
    {
        return DaneUczelni.Prowadzacy
            ...
            {
                x.ImieNazwisko,
                Srednia = x.Oceny.Any() ? x.Oceny.Average() : (double?)null
            })
            .OrderBy(x => x.ImieNazwisko)
            .Select(x => x.Srednia.HasValue
                ? $"{x.ImieNazwisko} | średnia ocen: {x.Srednia.Value:0.00}"
                : $"{x.ImieNazwisko} | średnia ocen: brak ocen");
    }
